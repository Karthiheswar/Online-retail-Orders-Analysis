...
#-------------------------------------------------------
  ...
  #query to display carton id, (len*width*height) as carton_vol and identify the optimum carton (carton with the least volume whose volume is greater than the total volume of all items (len * width * height * product_quantity)) for a given order whose order id is 10006
  
  select oi.order_id, c.carton_id, (c.len*c.width*c.height) as carton_vol, (c.len*c.width*c.height*oi.product_quantity)
  from carton c
  left join product p on p.width=c.width
  left join order_items oi on oi.product_id=p.product_id
  where oi.order_id = 10006
  
  #Q8
  
  #query to display details (customer id,customer fullname,order id,productquantity) of customers who bought more than ten (i.e. total order qty) products per order
  
  select oc.CUSTOMER_ID, oc.CUSTOMER_FNAME, oc.CUSTOMER_LNAME, oh.ORDER_ID, sum(oi.PRODUCT_QUANTITY)
  ...
  #query to display the order_id, customer id and cutomer full name of customers along with (product_quantity) as total quantity of products ordered for order ids > 10060
  
  select oh.ORDER_ID, oc.CUSTOMER_ID, oc.CUSTOMER_FNAME, oc.CUSTOMER_LNAME, sum(oi.PRODUCT_QUANTITY) as TOTAL_QUANTITY
  from online_customer oc 
  left join order_header oh on oc.CUSTOMER_ID=oh.CUSTOMER_ID
  left join order_items oi on oh.ORDER_ID=oi.ORDER_ID
  where oh.ORDER_STATUS = 'Shipped'
  ...
  #query to display country, product class description ,total quantity (sum(product_quantity),Total value (product_quantity * product price) and show which class of products have been shipped highest to countries outside India other than USA? Also show the total value of those items
  
  select ad.COUNTRY, pc.PRODUCT_CLASS_DESC, sum(oi.PRODUCT_QUANTITY) as TOTAL_QUANTITY, sum(oi.PRODUCT_QUANTITY*p.PRODUCT_PRICE) as TOTAL_VALUE
  FROM address ad
  INNER JOIN online_customer oc ON oc.ADDRESS_ID=ad.ADDRESS_ID
  ...
  WHERE ad.COUNTRY!='India' and ad.COUNTRY!='USA'
